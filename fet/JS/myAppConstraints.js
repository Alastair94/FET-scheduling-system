...
app.controller('ActivitiesCtrl', function( $scope, myHttp ) {
	...
	$scope.saveActPrefTime = function () {
		var activity_id = $scope.activity.id;
		myHttp.query({
			...
			if(data0[0]==null){
				...
				myHttp.query({
					...
					myHttp.query({
						...
						console.log('Succesfully created a new Preferred time constraint with id: ' + result + ', and ' + $scope.chosenTimes.length + ' preferred hours!');
					});
				});
			} else{
				window.alert("There is already a time constraint for activity with ID: " + activity_id);
			}
		})
	...
	$scope.saveActPrefSpace = function () {
		...
		var chosenSpaces = $scope.chosenSpaces;
		myHttp.query({
			...
			if(data0[0]==null){
				...
				myHttp.query({
					...
					myHttp.query({
						...
						console.log('Succesfully created a new Preferred space constraint with id: ' + result1 + ', and ' + chosenSpaces.length + ' preferred rooms!');
					});
				});
			} else{
				window.alert("There is already a space constraint for activity with ID: " + activity.id);
			}
		})
